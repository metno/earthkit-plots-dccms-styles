...
A translation of the DCCMS style library in
[`src/earthkit_plots_dccms_styles`](../src/earthkit_plots_dccms_styles) into the
style-library format used by [Magics](https://github.com/ecmwf/magics)
(`share/magics/styles/` in that repository).

Everything here is generated. To rebuild it after changing the YAML styles:
...
The deployable library lives in
[`share/magics/styles/dccms`](../share/magics/styles/dccms): a copy of the
stock ECMWF library with the DCCMS entries merged in. This folder holds the
generated pieces that feed it.

| File | Role | How Magics finds it |
|------|------|---------------------|
| `share/magics/styles/dccms/styles.json` | The 576 stock visual definitions plus the 12 DCCMS ones | `MAGICS_STYLE_PATH` |
| `share/magics/styles/dccms/dccms_<param>.json` | Match GRIB/NetCDF metadata to the DCCMS style names | `MAGICS_STYLE_PATH` |
| `magics/styles.json` | The 12 DCCMS definitions on their own, as generated | merged into the above |
| `magics/coastlines.json` | DCCMS map decoration (land, sea, coast, borders, grid) | `page_theme`, from the Magics installation |
| `magics/dccms_units-rules.json` | One extra unit conversion Magics does not ship | The Magics installation |
...
copying them alongside the stock library never silently replaces a stock file
of the same name (`2t.json`, `tp.json` and `msl.json` all exist upstream).
...
instead of an install, point it at `share/magics/styles/dccms` in the repo.
...
| `dccms_sh_2d_fM12t46i2` | `DEW_POINT_TEMPERATURE_IN_CELSIUS` | Dew point, °C |
...
| `dccms_sh_10si_f0t31lst` | `WIND_SPEED_AT_10M_IN_METERS_PER_SECOND` | 10m wind speed, m s⁻¹ |
| `dccms_sh_10si_f0t110lst` | `WIND_SPEED_AT_10M_IN_KM_PER_HOUR` | 10m wind speed, km h⁻¹ |
...
`grib_scaling_factor` in the style is honoured for this. `dccms_10si.json` therefore
...
then add `"prefered_units": "km/h"` to `dccms_10si.json` and move
`dccms_sh_10si_f0t110lst` to the front of its `styles` list.
...
copy `share/magics/styles/ecmwf/`, merge this folder's `styles.json` into its
`styles.json`, and drop the `dccms_*.json` parameter files in. The `dccms_`
prefix guarantees nothing is overwritten in the process.
...
`dccms_2d.json` matches paramId 3017 / `dpt` where stock matches 168 / `2d`,
`dccms_10si.json` matches paramId 10 / `ws` where stock matches 207 / `10si`,
